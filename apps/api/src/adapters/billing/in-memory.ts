...
export class InMemorySubscriptionRepository implements SubscriptionRepository {
  ...
  async create(userId: string, trialEndsAt: number): Promise<SubscriptionRecord> {
    const record: SubscriptionRecord = { userId, status: 'trialing', trialEndsAt, trialExtended: false, stripeCustomerId: null, stripeSubscriptionId: null, currentPeriodEnd: null, currentPeriodStart: null };
